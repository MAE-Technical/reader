...
export default function BookCard({ material }: { material: MaterialSummary }) {
  ...
  return (
    <Link href={`/book/${material.slug}`} className="group block no-underline">
      <div className="relative overflow-hidden rounded-xs border border-[var(--reader-border)]">
        <BookCover src={resolveBookThumbnailSrc(material)} alt={material.title} className="aspect-[2/3] w-full" />
        {showProgress && (
      ...
      <div className="mt-2.5 min-w-0">
        <div className="text-[12px] font-bold leading-tight text-[var(--reader-text)] group-hover:text-brand-500">
          {material.title}
        </div>
        <div className="mt-0.5 text-xs font-medium text-[var(--reader-text-muted)]">{material.author}</div>
      </div>
    </Link>
  );
}
